...
(module
  ...
  "#;
    ...
    let test_store_to_load = store
        .instance_export_checked(module, "test_store_to_load")
        .unwrap()
        .as_func()
        .unwrap();
    let zero_everything = store
        .instance_export_checked(module, "zero_everything")
        .unwrap()
        .as_func()
        .unwrap();
    let test_redundant_load = store
        .instance_export_checked(module, "test_redundant_load")
        .unwrap()
        .as_func()
        .unwrap();
    let test_dead_store = store
        .instance_export_checked(module, "test_dead_store")
        .unwrap()
        .as_func()
        .unwrap();
    let malloc_aliasing = store
        .instance_export_checked(module, "malloc_aliasing")
        .unwrap()
        .as_func()
        .unwrap();

    assert_eq!(
        store.invoke_typed_without_fuel_checked(test_store_to_load, ()),
        Ok(0x00000080)
    );
    store
        .invoke_typed_without_fuel_checked::<(), ()>(zero_everything, ())
        .unwrap();
    assert_eq!(
        store.invoke_typed_without_fuel_checked(test_redundant_load, ()),
        Ok(0x00000080)
    );
    store
        .invoke_typed_without_fuel_checked::<(), ()>(zero_everything, ())
        .unwrap();
    assert_eq!(
        store.invoke_typed_without_fuel_checked(test_dead_store, ()),
    ...
    store
        ...
        .unwrap();
    assert_eq!(
        store.invoke_typed_without_fuel_checked(malloc_aliasing, ()),
        Ok(43)
    );
}
