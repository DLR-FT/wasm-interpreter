use wasm::{global_store::GlobalStore, validate, RuntimeError, RuntimeInstance, DEFAULT_MODULE};
...
const EXPORT_GLOBAL_MODULE: &'static str = r#"
(module
  (global $my_global (export "my_global") i32 (i32.const 42))
)
"#;

const IMPORT_GLOBAL_MODULE: &'static str = r#"
(module
  (global $imported_global (import "export" "my_global") i32)

  (func $get_global (export "get_global") (result i32)
...
pub fn import_global() {
    ...
    let wasm_bytes = wat::parse_str(EXPORT_GLOBAL_MODULE).unwrap();
    let validation_info = validate(&wasm_bytes).expect("validation failed");
    let mut instance = RuntimeInstance::new_named("export", &validation_info, global_store)
        .expect("instantiation failed");

    let wasm_bytes = wat::parse_str(IMPORT_GLOBAL_MODULE).unwrap();
    let validation_info = validate(&wasm_bytes).expect("validation failed");
    instance
    ...
    let func = instance
        .get_function_by_name("import", "get_global")
        .unwrap();
