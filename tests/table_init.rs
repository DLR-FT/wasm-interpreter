...
fn table_init_1_test() {
    ...
    let test = store
        .instance_export_checked(module, "test")
        .unwrap()
        .as_func()
        .unwrap();
    let check = store
        .instance_export_checked(module, "check")
        .unwrap()
        .as_func()
        .unwrap();

    store
        .invoke_typed_without_fuel_checked::<(), ()>(test, ())
        .unwrap();

    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 0)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 1)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        3,
        store.invoke_typed_without_fuel_checked(check, 2).unwrap()
    );
    assert_eq!(
        1,
        store.invoke_typed_without_fuel_checked(check, 3).unwrap()
    );
    assert_eq!(
        4,
        store.invoke_typed_without_fuel_checked(check, 4).unwrap()
    );
    assert_eq!(
        1,
        store.invoke_typed_without_fuel_checked(check, 5).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 6)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        2,
        store.invoke_typed_without_fuel_checked(check, 7).unwrap()
    );
    assert_eq!(
        7,
        store.invoke_typed_without_fuel_checked(check, 8).unwrap()
    );
    assert_eq!(
        1,
        store.invoke_typed_without_fuel_checked(check, 9).unwrap()
    );
    assert_eq!(
        8,
        store.invoke_typed_without_fuel_checked(check, 10).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 11)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        7,
        store.invoke_typed_without_fuel_checked(check, 12).unwrap()
    );
    assert_eq!(
        5,
        store.invoke_typed_without_fuel_checked(check, 13).unwrap()
    );
    assert_eq!(
        2,
        store.invoke_typed_without_fuel_checked(check, 14).unwrap()
    );
    assert_eq!(
        3,
        store.invoke_typed_without_fuel_checked(check, 15).unwrap()
    );
    assert_eq!(
        6,
        store.invoke_typed_without_fuel_checked(check, 16).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 17)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 18)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 19)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 20)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 21)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 22)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 23)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 24)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 25)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 26)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 27)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 28)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 29)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
}
...
(module
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 7)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 8)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 9)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 10)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
    ...
    assert_eq!(
        7,
        store.invoke_typed_without_fuel_checked(check, 17).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 18)
...
(module
  ...
  (func (export "check") (param i32) (result i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 13)
            .err(),
        Some(RuntimeError::Trap(TrapError::UninitializedElement))
    );
    assert_eq!(
    ...
    assert_eq!(
        9,
        store.invoke_typed_without_fuel_checked(check, 19).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 20)
    ...
    assert_eq!(
        7,
        store.invoke_typed_without_fuel_checked(check, 21).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 22)
    ...
    assert_eq!(
        8,
        store.invoke_typed_without_fuel_checked(check, 23).unwrap()
    );
    assert_eq!(
        8,
        store.invoke_typed_without_fuel_checked(check, 24).unwrap()
    );
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<i32, i32>(check, 25)
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    let run = store
        .instance_export_checked(module, "run")
        .unwrap()
        .as_func()
        .unwrap();
    let test = store
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, i32), ()>(run, (24, 16))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    ...
    for i in 0..32 {
        assert_eq!(
            store
                .invoke_typed_without_fuel_checked::<i32, i32>(test, i)
                .err(),
            Some(RuntimeError::Trap(TrapError::UninitializedElement))
        );
    }
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, i32), ()>(run, (25, 16))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, i32), ()>(run, (96, 32))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, i32), ()>(run, (97, 31))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, u32), ()>(run, (48, 4294967280_u32))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    assert_eq!(
        store
            .invoke_typed_without_fuel_checked::<(i32, i32), ()>(run, (0, 4294967292_u32 as i32))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        )),
