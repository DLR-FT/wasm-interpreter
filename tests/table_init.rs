...
use wasm::{
    hooks::EmptyHookSet, validate, RuntimeError, RuntimeInstance, TrapError, ValidationError,
    DEFAULT_MODULE,
};
...
fn table_init_1_test() {
    ...
    assert!(
        i.invoke_typed::<i32, i32>(check, 0).err().unwrap()
            == RuntimeError::Trap(TrapError::UninitializedElement).into()
    );
    assert!(
...
(module
  ...
  (func (export "run") (param $offs i32) (param $len i32)
    ...
    let wasm_bytes = wat::parse_str(w).unwrap();
    let validation_info = validate(&wasm_bytes).unwrap();
    let mut inst = RuntimeInstance::<'_, (), EmptyHookSet, Infallible>::new_with_default_module(
        (),
        &validation_info,
    )
    .unwrap();
