...
pub fn i32_add() {
    ...
    assert_eq!(2, store.invoke_simple_typed(function, (1, 1)).unwrap());
    assert_eq!(1, store.invoke_simple_typed(function, (1, 0)).unwrap());
    assert_eq!(-2, store.invoke_simple_typed(function, (-1, -1)).unwrap());
    assert_eq!(0, store.invoke_simple_typed(function, (-1, 1)).unwrap());
    ...
    assert_eq!(
        i32_min,
        store
            .invoke_simple_typed(function, (0x7fffffff, 1))
            .unwrap()
    );
    assert_eq!(
        0x7fffffff,
        store.invoke_simple_typed(function, (i32_min, -1)).unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (i32_min, i32_min))
            .unwrap()
    );
    assert_eq!(
        0x40000000,
        store
            .invoke_simple_typed(function, (0x3fffffff, 1))
            .unwrap()
    );
}
...
"#;
    ...
    assert_eq!(1, store.invoke_simple_typed(i32_eqz, ()).unwrap());
}
...
pub fn i32_eqz() {
    ...
    assert_eq!(1, store.invoke_simple_typed(i32_eqz, 0).unwrap());
    assert_eq!(0, store.invoke_simple_typed(i32_eqz, 1).unwrap());
    assert_eq!(
        0,
        store
            .invoke_simple_typed(i32_eqz, 0x80000000u32 as i32)
            .unwrap()
    );
    assert_eq!(0, store.invoke_simple_typed(i32_eqz, 0x7fffffff).unwrap());
    assert_eq!(
        0,
        store
            .invoke_simple_typed(i32_eqz, 0xffffffffu32 as i32)
            .unwrap()
    );
}
...
"#;
    ...
    assert_eq!(1, store.invoke_simple_typed(i32_eq, ()).unwrap());
}
...
pub fn i32_eq() {
    ...
    assert_eq!(1, store.invoke_simple_typed(function, (0, 0)).unwrap());
    assert_eq!(1, store.invoke_simple_typed(function, (1, 1)).unwrap());
    assert_eq!(0, store.invoke_simple_typed(function, (-1, 1)).unwrap());
    assert_eq!(
        1,
        store
            .invoke_simple_typed(function, (0x80000000u32 as i32, 0x80000000u32 as i32))
            .unwrap()
    );
    assert_eq!(
        1,
        store
            .invoke_simple_typed(function, (0x7fffffff, 0x7fffffff))
            .unwrap()
    );
    assert_eq!(1, store.invoke_simple_typed(function, (-1, -1)).unwrap());
    assert_eq!(0, store.invoke_simple_typed(function, (1, 0)).unwrap());
    assert_eq!(0, store.invoke_simple_typed(function, (0, 1)).unwrap());
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (0x80000000u32 as i32, 0))
            .unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (0, 0x80000000u32 as i32))
            .unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (0x80000000u32 as i32, -1))
            .unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (-1, 0x80000000u32 as i32))
            .unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (0x80000000u32 as i32, 0x7fffffff))
            .unwrap()
    );
    assert_eq!(
        0,
        store
            .invoke_simple_typed(function, (0x7fffffff, 0x80000000u32 as i32))
            .unwrap()
    );
}
...
pub fn i32_ne() {
    ...
    assert_eq!(0, store.invoke_simple_typed(function, (-1, -1)).unwrap());
    assert_eq!(1, store.invoke_simple_typed(function, (1, 0)).unwrap());
    assert_eq!(1, store.invoke_simple_typed(function, (0, 1)).unwrap());
    assert_eq!(
        1,
        store
            .invoke_simple_typed(function, (0x80000000u32 as i32, 0))
