...
use interop::{StoreTypedInvocationExt, host_function_wrapper};
use wasm::{
    ExternVal, HaltExecutionError, RuntimeError, Store, Value, validate,
    value::{F32, F64},
};
...
pub fn simple_multivariate_host_func_within_module() {
    ...
    // correct store.
    let result =
        unsafe { store.invoke_simple_typed_unchecked::<(), (f64, i32, i64)>(function_ref, ()) }
            .expect("wasm function invocation failed");
...
pub fn simple_multivariate_host_func_as_first_func() {
    ...
    // correct store.
    let result = unsafe {
        store.invoke_simple_typed_unchecked::<(i32, f64), (f64, i32)>(fancy_add_mult, (3, 5.0))
    }
    .expect("wasm function invocation failed");
...
pub fn weird_multi_typed_host_func() {
    ...
    // SAFETY: Only one store exists in this test. Therefore, it is always the
    // correct store.
    let result = unsafe { store.invoke_simple_typed_unchecked::<(), (f64, i64)>(function_ref, ()) }
        .expect("wasm function invocation failed");
