...
pub fn validation_error_to_wasm_testsuite_string(
    ...
    match validation_error {
        ...
        ValidationError::VariableLengthIntegerOverflowed => Ok("integer too long"),
        ...
        ValidationError::MalformedSpan => Ok("section size mismatch"),
        ValidationError::InvalidTypeIdx(type_idx) => return Ok(format!("unknown type {type_idx}")),
        ValidationError::InvalidFuncIdx(func_idx) => return Ok(format!("unknown function {func_idx}")),
        ValidationError::InvalidTableIdx(table_idx) => return Ok(format!("unknown table {table_idx}")),
        ValidationError::InvalidMemIdx(mem_idx) => return Ok(format!("unknown memory {mem_idx}")),
        ValidationError::InvalidGlobalIdx(global_idx) => return Ok(format!("unknown global {global_idx}")),
        ValidationError::InvalidElemIdx(elem_idx) => return Ok(format!("unknown elem segment {elem_idx}")),
        ValidationError::InvalidDataIdx(data_idx) => return Ok(format!("unknown data segment {data_idx}")),
        ValidationError::InvalidLocalIdx(local_idx) => return Ok(format!("unknown local {local_idx}")),
        ValidationError::InvalidLabelIdx(label_idx) => return Ok(format!("unknown label {label_idx}")),
        ...
        ValidationError::ExpectedZeroByte => {
            Ok("zero byte expected")
        }
        ...
        ValidationError::MalformedLimitsDiscriminator(_) => Err(
            WastError::UnrepresentedValidationError(validation_error.clone()),
        ),
