    ...
    for directive in wast.directives {
        match directive {
            wast::WastDirective::Wat(mut quoted) => {
                ...
                let validation_info =
                    try_to!(validate(wasm_bytes.as_ref().unwrap()).map_err(|err| {
                        WastError::from_outside(
    ...
    match exec {
        wast::WastExecute::Invoke(invoke_info) => {
            ...
            let actual = interpeter
                ...
                .map_err(|err| Box::<WasmInterpreterError>::new(err.into()))?;
...
fn result_to_value(result: wast::WastRet) -> Value {
    match result {
        wast::WastRet::Core(core_arg) => match core_arg {
            ...
            wast::core::WastRetCore::F32(_) => todo!(),
            wast::core::WastRetCore::F64(_) => todo!(),
