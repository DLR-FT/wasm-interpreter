...
fn table_grow_test() {
    ...
    assert_eq!(store.invoke_simple_typed::<(), i32>(size, ()), Ok(0));
    assert_eq!(
        store
            .invoke_simple_typed::<(i32, RefExtern), ()>(set, (0, RefExtern(Some(ExternAddr(2)))))
            .err(),
    ...
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 0).err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    );
    ...
    assert_eq!(
        store.invoke_simple_typed::<(i32, RefExtern), i32>(grow, (1, RefExtern(None))),
        Ok(0)
    );
    assert_eq!(store.invoke_simple_typed::<(), i32>(size, ()), Ok(1));
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 0),
        Ok(RefExtern(None))
    );
    assert_eq!(
        store.invoke_simple_typed::<(i32, RefExtern), ()>(set, (0, RefExtern(Some(ExternAddr(2))))),
        Ok(())
    );
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 0),
    ...
    assert_eq!(
        store
            .invoke_simple_typed::<(i32, RefExtern), ()>(set, (1, RefExtern(Some(ExternAddr(2)))))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    ...
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 1).err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    );
    ...
    assert_eq!(
        store.invoke_simple_typed::<(i32, RefExtern), i32>(
            grow_abbrev,
            (4, RefExtern(Some(ExternAddr(3))))
        ),
        Ok(1)
    );
    assert_eq!(store.invoke_simple_typed::<(), i32>(size, ()), Ok(5));
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 0),
        Ok(RefExtern(Some(ExternAddr(2))))
    );
    ...
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 1),
        Ok(RefExtern(Some(ExternAddr(3))))
    );
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 4),
        Ok(RefExtern(Some(ExternAddr(3))))
    );
    assert_eq!(
        store.invoke_simple_typed::<(i32, RefExtern), ()>(set, (4, RefExtern(Some(ExternAddr(4))))),
        Ok(())
    );
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 4),
    ...
    assert_eq!(
        store
            .invoke_simple_typed::<(i32, RefExtern), ()>(set, (5, RefExtern(Some(ExternAddr(2)))))
            .err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    ...
    assert_eq!(
        store.invoke_simple_typed::<i32, RefExtern>(get, 5).err(),
        Some(RuntimeError::Trap(
            TrapError::TableOrElementAccessOutOfBounds
        ))
    );
...
fn table_grow_outside_i32_range() {
    ...
    let grow = store
        ...
        .as_func()
        .unwrap();
    assert_eq!(store.invoke_simple_typed::<(), i32>(grow, ()).unwrap(), -1);
}
...
fn table_grow_unlimited() {
    ...
    let grow = store
        ...
        .as_func()
        .unwrap();
    assert_eq!(store.invoke_simple_typed(grow, 0), Ok(0));
    assert_eq!(store.invoke_simple_typed(grow, 1), Ok(0));
    assert_eq!(store.invoke_simple_typed(grow, 0), Ok(1));
    assert_eq!(store.invoke_simple_typed(grow, 2), Ok(1));
    assert_eq!(store.invoke_simple_typed(grow, 800), Ok(3));
}
...
fn table_grow_with_max() {
    ...
    assert_eq!(store.invoke_simple_typed(grow, 2), Ok(2));
    assert_eq!(store.invoke_simple_typed(grow, 6), Ok(4));
...
fn table_grow_check_null() {
    ...
    assert_eq!(
        store
            .invoke_simple_typed::<(i32, i32), RefExtern>(check_table_null, (0, 9))
            .unwrap(),
        RefExtern(None)
    );
    assert_eq!(store.invoke_simple_typed(grow, 10), Ok(10));
    assert_eq!(
        store
            .invoke_simple_typed::<(i32, i32), RefExtern>(check_table_null, (0, 19))
            .unwrap(),
        RefExtern(None)
    );
}
...
fn table_grow_with_exported_table_test() {
    ...
    let grow = store
        ...
        .as_func()
        .unwrap();
    assert_eq!(store.invoke_simple_typed(grow, ()), Ok(1));
}
