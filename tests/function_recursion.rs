...
fn simple_function_call() {
    ...
    let simple_caller = store
        .instance_export_checked(module, "simple_caller")
        .unwrap()
        .as_func()
        .unwrap();
    ...
    assert_eq!(
        3 * 7 + 13,
        store
            .invoke_typed_without_fuel_checked(simple_caller, (3, 7))
            .unwrap()
    );
}
...
fn recursion_valid() {
    ...
    assert_eq!(
        12,
        store
            .invoke_typed_without_fuel_checked(add_two, 10)
            .unwrap()
    );
    assert_eq!(
        2,
        store.invoke_typed_without_fuel_checked(add_two, 0).unwrap()
    );
    assert_eq!(
        -4,
        store
...
fn multivalue_call() {
    ...
    let foo = store
        .instance_export_checked(module, "bar")
        .unwrap()
        .as_func()
        .unwrap();
    ...
    assert_eq!(
        (10, 42.0, 5),
        store
            .invoke_typed_without_fuel_checked::<(), (i32, f32, i64)>(foo, ())
            .unwrap()
    );
}
