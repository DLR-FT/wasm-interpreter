...
fn polymorphic_select_test() {
    ...
    let select_test = store
        .instance_export_checked(module, "select_test")
        .unwrap()
        .as_func()
        .unwrap();
    ...
    assert_eq!(
        4,
        store
            .invoke_typed_without_fuel_checked(select_test, 0)
            .unwrap()
    );
    assert_eq!(
        8,
        store
            .invoke_typed_without_fuel_checked(select_test, 1)
            .unwrap()
    );
    assert_eq!(
        15,
        store
            .invoke_typed_without_fuel_checked(select_test, 2)
            .unwrap()
    );
    assert_eq!(
        16,
        store
            .invoke_typed_without_fuel_checked(select_test, 3)
            .unwrap()
    );
}
