...
fn simple_byte_writes() {
    ...
    let mem_as_slice = store.mem_data_mut(mem).expect("this memory is unshared");
...
fn interpret_as_str() {
    ...
    let mut mem_as_slice = store.mem_data_mut(mem).expect("this memory is unshared");

    let bytes_written = mem_as_slice.write(STR_TO_WRITE.as_bytes()).unwrap();
    assert_eq!(bytes_written, 12);

    let mem_as_slice = store.mem_data(mem).expect("this memory is unshared");
