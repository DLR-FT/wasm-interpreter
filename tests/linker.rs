...
pub fn compile_simple_import() {
    ...
    let validation_info_addon = decode_and_validate(&wasm_bytes_addon).expect("validation failed");

    let wasm_bytes_base = wat::parse_str(SIMPLE_IMPORT_BASE).unwrap();
    let validation_info_base = decode_and_validate(&wasm_bytes_base).expect("validation failed");
