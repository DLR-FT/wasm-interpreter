...
fn i32_add_one() {
    ...
    assert_eq!(
        12,
        store
            .invoke_typed_without_fuel_checked(add_one, 11)
            .unwrap()
    );
    assert_eq!(
        1,
        store.invoke_typed_without_fuel_checked(add_one, 0).unwrap()
    );
    assert_eq!(
        -5,
        store
...
fn i64_add_one() {
    ...
    assert_eq!(
        12_i64,
        store
            .invoke_typed_without_fuel_checked(add_one, 11_i64)
            .unwrap()
    );
    assert_eq!(
        1_i64,
        store
            .invoke_typed_without_fuel_checked(add_one, 0_i64)
            .unwrap()
    );
    assert_eq!(
        -5_i64,
        store
            .invoke_typed_without_fuel_checked(add_one, -6_i64)
            .unwrap()
    );
}
