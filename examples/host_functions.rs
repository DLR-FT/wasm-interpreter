...
//!
//! - alternative execution schemes (e.g. exiting the intepreter is much easier compared to doing it
//!   within a callback)
//!
...
//!
//! In this example, we register two host functions to the store, `host_read_num` and
//! `host_print_num`, which read an i32 from stdin and print an i32 to stdout respectively. We then
//! import these host functions within our Wasm module where we do simple arithmethic (with
//! ill-conditions ignored for the simplicity of the example).
...
(module $wasm_src.wasm
    (import "host" "host_read_num" (func $read_num (param) (result i32)))
    (import "host" "host_print_num" (func $print_num (param i32) (result)))
...
/// Our custom pick for hostcode value of `host_read_num`. Hostcodes are arbitrary usize values we
/// pick for host functions to identify them later. Any choice for `READ_NUM_HOSTCODE` is fine as
/// long as it does not clash with other hostcodes.
...
/// Our custom pick for hostcode value of `host_print_num`.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Wasm type of `host_read_num` is () -> (i32).
    ...
    // Wasm type of `host_print_num` is (i32) -> ().
    // It is the developers responsibility to adhere to the output type signature specified in the
    // `returns` field.
    ...
    // Since `host_read_num` was declared first as an import, it precedes `host_print_num`.
    ...
    // Therefore handling host functions are very similar to handling resumables that are out of
    // fuel (see `examples/fuel.rs`). The only difference here is the `run_state` returned by the
    // invocation possibly contains a hostcode that was associated to the host function that was
    // just invoked.

    // Specifically, there are 3 possibilities, corresponding to each enum variant of the `RunState`
    // type:
    loop {
        match run_state {
