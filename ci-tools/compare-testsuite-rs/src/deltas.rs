...
impl FileDeltaType<'_> {
    fn get_icon(&self) -> &'static str {
        const GOOD: &str = ":white_check_mark:";
        const BAD: &str = ":x:";
        const WARN: &str = ":warning:";

        match self {
            FileDeltaType::NewScriptError { .. } => BAD,
            FileDeltaType::ScriptErrorResolved { .. } => GOOD,
            FileDeltaType::ScriptErrorChanged { .. } => WARN,
            FileDeltaType::AssertsPassing { .. } => GOOD,
            FileDeltaType::AssertsFailing { .. } => BAD,
            FileDeltaType::AssertsChanged { .. } => WARN,
        }
    }
}
        ...
        match &self.ty {
            FileDeltaType::NewScriptError {
                ...
                write_details_summary(
                    ...
                    |f| {
                        ...
                        write!(f, "**Now**<br>")?;
                        write!(f, "- Line: {}<br>", to_str_or_placeholder(*new_line_number))?;
                        write!(f, "- Error: {new_error}<br>")?;
                        write!(f, "- Context: {new_context}")
                    },
                )?;
                write!(f, " |")
            }
            FileDeltaType::ScriptErrorResolved {
                ...
                write_details_summary(
                    ...
                    |f| {
                        write!(f, "**Before**<br>")?;
                        write!(f, "- Line {}<br>", to_str_or_placeholder(*old_line_number))?;
                        write!(f, "- Error: {old_error}<br>")?;
                        write!(f, "- Context: {old_context}")
                    },
                )?;
            ...
            FileDeltaType::ScriptErrorChanged {
                ...
                write_details_summary(
                    ...
                    |f| {
                        ...
                        write!(f, "- Line {}<br>", to_str_or_placeholder(*old_line_number))?;
                        write!(f, "- Error: `{old_error}`<br>")?;
                        write!(f, "- Context: {old_context}<br>")?;
            ...
            FileDeltaType::AssertsPassing {
                ...
                write_details_summary(
                    ...
                    |f| {
                        now_passing
                            .iter()
                            .try_for_each(|assert| write!(f, "- Line {}<br>", assert.line_number))
                    },
                )?;
                ...
                write_details_summary(
                    ...
                    |f| {
                        now_failing.iter().try_for_each(|assert| {
                            write!(f, "- Line {}: {}<br>", assert.line_number, assert.new_error)
                        })
                    },
                )?;
...
fn write_script_error() {}
fn write_passing_ratio(_delta: Option<()>) {}
fn write_passing_percentage(_delta: Option<()>) {}
