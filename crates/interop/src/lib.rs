...
/// Helper function to quickly convert parameters and result values back and
/// forth for implementing host functions.
...
pub trait StoreTypedInvocationExt<T: Config> {
    ...
    fn func_alloc_typed_unchecked<Params: InteropValueList, Returns: InteropValueList>(
        &mut self,
        hostcode: Hostcode,
    ) -> FuncAddr;
