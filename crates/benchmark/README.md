...
This crate contains multiple benchmarks:

- The primary benchmark we use is the [CoreMark benchmark ported to
  WebAssembly](https://github.com/wasm3/wasm-coremark) by the wasm3 developers. It consists of a
  single file `benches/coremark_minimal/coremark-minimal.wasm` and is provided without any license.

  It is based on the [CoreMark](https://github.com/eembc/coremark) benchmark by EEMBC, which is
  provided under the COREMARK® ACCEPTABLE USE AGREEMENT and the Apache License 2.0, which are
  included below.

- Microbenchmarks for various LEB128 integer reading algorithms
- General purpose benchmark for interpreter speed. Consists of the following bench-functions:
  - fibonacci_recursive: benches function creation, as due to its recursive nature new call-frames
    are created and removed very often.
...
```sh
# Run coremark
cargo bench --bench coremark_minimal

# General-purpose benchmarks for just our interpreter
cargo bench --bench general_purpose

# General-purpose benchmarks for our interpreter vs. wasmi vs. wasmtime
cargo bench --features wasmi,wasmtime --bench general_purpose

# Analyze the impact of changes on your current branch (including unstaged modifications!) vs main based on the general-purpose benchmarks
bench-against-main
```

## Copy of CoreMark's Acceptable Use Agreement and License
