...
pub fn validate(wasm: &[u8]) -> Result<ValidationInfo<'_>, ValidationError> {
    ...
    let start = handle_section(&mut wasm, &mut header, SectionTy::Start, |wasm, _| {
        ...
        // Safety: There exists only one types vector in this function.
        // Therefore, this has to be the same one used to create the `TypeIdx`.
    ...
    let func_blocks_stps = handle_section(&mut wasm, &mut header, SectionTy::Code, |wasm, h| {
        // Safety: There  exists only one types vector in this function.
        // Therefore, it has to be the same one used to create all `TypeIdx`
        // objects that are also passed here.
        unsafe {
