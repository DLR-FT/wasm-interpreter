...
/// # Safety
///
/// The caller must ensure that all [`TypeIdx`] objects passed to this function
/// belong to the [`IdxVec<TypeIdx, FuncType>`] vector that is also passed.
...
pub unsafe fn validate_code_section(
    ...
    let code_block_spans_stps = wasm.read_vec_enumerated(|wasm, idx| {
        ...
        // SAFETY: The caller guarantees that all all `TypeIdx` objects,
        // including `ty_idx`, comes from the `fn_types` vector.
        ...
        // SAFETY: The caller ensures equal safety requirements and no new
        // `TypeIdx` objects were created.
        unsafe {
            read_instructions(
                wasm,
                &mut stack,
