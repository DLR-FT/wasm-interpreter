...
//! There are addr types for different index spaces, such as memories, globals or functions [`FuncAddr`].
//!
//!
//! # A Note About Accessor Methods on Store Address Spaces
//! At first, we stored a [`Vec`](alloc::vec::Vec) directly in the [`Store`](crate::Store) for function instances, table instances, etc.
//! However, implementing accessor methods on the [`Store`](crate::Store) causes problems,
//! because either the entire [`Store`](crate::Store) has to be passed as an argument
//! (preventing partial borrows) or a specific [`Vec`](alloc::vec::Vec) has to be passed as an
//! argument (exposing [`Store`](crate::Store) implementation details through a pretty unergonomic
//! API).
//!
//! Because both of these solutions were not sufficient, a choice was made for newtype wrappers around every address space.
//! This way, partial borrows of the [`Store`](crate::Store) are possible, while providing a nice API.
...
/// An address to a [`MemInst`] that lives in a specific [`Store`](crate::Store).
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
...
/// An address to a [`GlobalInst`] that lives in a specific [`Store`](crate::Store).
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
...
/// An address to a [`DataInst`] that lives in a specific [`Store`](crate::Store).
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
...
/// An address to a [`ModuleInst`] that lives in a specific [`Store`](crate::Store).
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
