...
#[inline(always)]
pub unsafe fn nop(_: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
...
#[inline(always)]
pub unsafe fn unreachable(_: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    Err(TrapError::ReachedUnreachable.into())
}

define_instruction!(super::block, block_mod, fuel_check = flat(BLOCK));
#[inline(always)]
pub unsafe fn block(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
...
#[inline(always)]
pub unsafe fn end(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    ...
    let current_function = unsafe {
        args.store_inner
            .functions
            .get(args.resumable.current_func_addr)
    };
...
#[inline(always)]
pub unsafe fn r#loop(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
...
#[inline(always)]
pub unsafe fn r#if(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    // SAFETY: Validation guarantees there to be a valid block type
    // next.
...
#[inline(always)]
pub unsafe fn r#else(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    do_sidetable_control_transfer(
        args.wasm,
        &mut args.resumable.stack,
        &mut args.resumable.stp,
...
#[inline(always)]
pub unsafe fn br(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
...
#[inline(always)]
pub unsafe fn br_if(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    // SAFETY: Validation guarantees there to be a valid label index
    // next.
    let _label_idx = unsafe { decode_label_idx_unchecked(args.wasm) };
...
#[inline(always)]
pub unsafe fn br_table(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    let label_vec = args
        .wasm
...
#[inline(always)]
pub unsafe fn r#return(args: Args) -> Result<ControlFlow<InterpreterLoopOutcome>, RuntimeError> {
    // same as BR
    do_sidetable_control_transfer(
        args.wasm,
        &mut args.resumable.stack,
