...
//!
//! While this is easy for address types like [`FuncAddr`] or [`MemAddr`], some types are enums and their variants are visible to the user.
//! For example, consider the [`Value`] enum, where users have full access to all of its variants. To be able to attach a tag only to the [`Value::Ref`] variant of this enum, the entire enum has to be re-defined.
//! The result is a completely new type [`StoredValue`].
//!
//! To allow all of these stored objects to be handled in a similar way, the trait [`AbstractStored`] is defined.
...
// defined in for the unchecked [`Store`] methods. Also all functions must
...
/// Helper method for associating every [`Value`] in a [`Vec`] with a
/// [`StoreId`], producing a [`Vec<StoredValue>`].
///
/// # Safety
///
/// It must be guaranteed that all given [`Value`]s come from the [`Store`] with
/// the given [`StoreId`].
// TODO safety comment is not correct. not the values must come from some store,
// but the addr types, if there are any.
