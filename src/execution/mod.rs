...
pub struct InstantiatedInstance<'bytecode, 'validation> {
    validated: &'validation ValidationInfo<'bytecode>,
}

pub fn instantiate<'b, 'v>(
    validation_info: &'v ValidationInfo<'b>,
) -> Result<InstantiatedInstance<'b, 'v>> {
...
pub fn invoke_func<Param: WasmValueList, Returns: WasmValueList>(
    ...
    let fn_code_span = *instantiation
        .validated
        .code_blocks
        .get(fn_idx)
        .expect("valid fn_idx");
    ...
    let mut wasm = WasmReader::new(instantiation.validated.wasm);
