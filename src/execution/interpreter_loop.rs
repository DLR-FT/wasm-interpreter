...
use crate::{
    ...
    value::{self, FuncAddr, InteropValue, Ref, F32, F64},
    value_stack::Stack,
        ...
        match first_instr_byte {
            ...
            FD_EXTENSIONS => {
                ...
                match second_instr {
                    ...
                    V128_AND => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let data1: [u8; 16] = stack.pop_value(ValType::VecType).into();
                    ...
                    V128_BITSELECT => {
                        let data3: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let data1: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let result =
                    ...
                    // shape.splat
                    I8X16_SPLAT => {
                        let value: u32 = stack.pop_value(ValType::NumType(NumType::I32)).into();
                    ...
                    I32X4_SPLAT => {
                        let lane: u32 = stack.pop_value(ValType::NumType(NumType::I32)).into();
                        let data = from_lanes([lane; 4]);
                        stack.push_value(Value::V128(data))?;
                    }
                    I64X2_SPLAT => {
                        let lane: u64 = stack.pop_value(ValType::NumType(NumType::I64)).into();
                        let data = from_lanes([lane; 2]);
                        stack.push_value(Value::V128(data))?;
                    }
                    F32X4_SPLAT => {
                        let lane: f32 = stack.pop_value(ValType::NumType(NumType::F32)).into();
                        let data = from_lanes([lane; 4]);
                        stack.push_value(Value::V128(data))?;
                    }
                    F64X2_SPLAT => {
                        let lane: f64 = stack.pop_value(ValType::NumType(NumType::F64)).into();
                        let data = from_lanes([lane; 2]);
                        stack.push_value(Value::V128(data))?;
                    }
                    ...
                    F32X4_EXTRACT_LANE => {
                        ...
                        let lane = *lanes.get(lane_idx).unwrap_validated();
                        stack.push_value(Value::F32(F32(lane)))?;
                    }
                    F64X2_EXTRACT_LANE => {
                        let lane_idx = wasm.read_u8().unwrap_validated() as usize;
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lane = *lanes.get(lane_idx).unwrap_validated();
                        stack.push_value(Value::F64(F64(lane)))?;
                    }
                    ...
                    I32X4_REPLACE_LANE => {
                        let lane_idx = wasm.read_u8().unwrap_validated() as usize;
                        let new_lane: u32 = stack.pop_value(ValType::NumType(NumType::I32)).into();
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let mut lanes: [u32; 4] = to_lanes(data);
                        *lanes.get_mut(lane_idx).unwrap_validated() = new_lane;
                        stack.push_value(Value::V128(from_lanes(lanes)))?;
                    ...
                    I64X2_REPLACE_LANE => {
                        let lane_idx = wasm.read_u8().unwrap_validated() as usize;
                        let new_lane: u64 = stack.pop_value(ValType::NumType(NumType::I64)).into();
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let mut lanes: [u64; 2] = to_lanes(data);
                        *lanes.get_mut(lane_idx).unwrap_validated() = new_lane;
                        stack.push_value(Value::V128(from_lanes(lanes)))?;
                    ...
                    F32X4_REPLACE_LANE => {
                        let lane_idx = wasm.read_u8().unwrap_validated() as usize;
                        let new_lane: f32 = stack.pop_value(ValType::NumType(NumType::F32)).into();
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                    ...
                    F64X2_REPLACE_LANE => {
                        let lane_idx = wasm.read_u8().unwrap_validated() as usize;
                        let new_lane: f64 = stack.pop_value(ValType::NumType(NumType::F64)).into();
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                    ...
                    F32X4_ABS => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).abs().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_ABS => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).abs().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_NEG => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).neg().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_NEG => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).neg().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_SQRT => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).sqrt().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_SQRT => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).sqrt().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_CEIL => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).ceil().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_CEIL => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).ceil().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_FLOOR => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).floor().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_FLOOR => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).floor().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_TRUNC => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).trunc().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_TRUNC => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).trunc().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_NEAREST => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| F32(lane).nearest().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_NEAREST => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [f64; 2] = to_lanes(data);
                        let result: [f64; 2] = lanes.map(|lane| F64(lane).nearest().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    // others
                    I8X16_POPCNT => {
                    ...
                    F32X4_ADD => {
                        ...
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).add(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_ADD => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).add(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_SUB => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).sub(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_SUB => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).sub(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_MUL => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).mul(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_MUL => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).mul(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_DIV => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).div(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_DIV => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).div(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_MIN => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).min(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_MIN => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).min(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_MAX => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [f32; 4] =
                            array::from_fn(|i| F32(lanes1[i]).max(F32(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_MAX => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [f64; 2] =
                            array::from_fn(|i| F64(lanes1[i]).max(F64(lanes2[i])).0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_PMIN => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                    ...
                    F32X4_EQ => {
                        ...
                        let lanes1: [f32; 4] = to_lanes(data1);
                        let result: [i32; 4] =
                            array::from_fn(|i| ((F32(lanes1[i]) == F32(lanes2[i])) as i32).neg());
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_EQ => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        ...
                        let lanes1: [f64; 2] = to_lanes(data1);
                        let result: [i64; 2] =
                            array::from_fn(|i| ((F64(lanes1[i]) == F64(lanes2[i])) as i64).neg());
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F32X4_NE => {
                        let data2: [u8; 16] = stack.pop_value(ValType::VecType).into();
                    ...
                    // Group vishiftop
                    I8X16_SHL => {
                        let shift: u32 = stack.pop_value(ValType::NumType(NumType::I32)).into();
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [u8; 16] = to_lanes(data);
                    ...
                    F32X4_CONVERT_I32X4_S => {
                        let data: [u8; 16] = stack.pop_value(ValType::VecType).into();
                        let lanes: [i32; 4] = to_lanes(data);
                        let result: [f32; 4] = lanes.map(|lane| lane as f32);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    ...
                    F64X2_CONVERT_LOW_I32X4_S => {
                        ...
                        let lanes: [i32; 4] = to_lanes(data);
                        let low_lanes: [i32; 2] = lanes[..2].try_into().unwrap();
                        let result = low_lanes.map(|lane| lane as f64);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    ...
                    F32X4_DEMOTE_F64X2_ZERO => {
                        ...
                        let half_lanes = lanes.map(|lane| F64(lane).as_f32().0);
                        let result = [half_lanes[0], half_lanes[1], 0.0, 0.0];
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
                    F64X2_PROMOTE_LOW_F32X4 => {
                        ...
                        let result = half_lanes.map(|lane| F32(lane).as_f64().0);
                        stack.push_value(Value::V128(from_lanes(result)))?;
                    }
