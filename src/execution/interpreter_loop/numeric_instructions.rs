...
define_instruction!(
    ...
    |Args { wasm, .. }: &mut Args<T>| {
        let constant = wasm.get_reader().read_var_i32().unwrap_validated();
        trace!("Instruction: i32.const [] -> [{constant}]");
        wasm.resumable.stack.push_value::<T>(constant.into())?;
        Ok(None)
    }
);
...
define_instruction!(
    ...
    opcode::I64_CONST,
    |Args { wasm, .. }: &mut Args<T>| {
        let constant = wasm.get_reader().read_var_i64().unwrap_validated();
...
define_instruction!(
    ...
    opcode::F32_CONST,
    |Args { wasm, .. }: &mut Args<T>| {
        let constant = F32::from_bits(wasm.get_reader().read_f32().unwrap_validated());
...
define_instruction!(
    ...
    opcode::F64_CONST,
    |Args { wasm, .. }: &mut Args<T>| {
        let constant = F64::from_bits(wasm.get_reader().read_f64().unwrap_validated());
...
// i32.unop
define_instruction!(i32_clz, opcode::I32_CLZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
    ...
    wasm.resumable.stack.push_value::<T>(res.into())?;
    Ok(None)
});

define_instruction!(i32_ctz, opcode::I32_CTZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
// i64.unop
define_instruction!(i64_clz, opcode::I64_CLZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i64 = wasm
        .resumable
...
define_instruction!(i64_ctz, opcode::I64_CTZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i64 = wasm
        .resumable
...
// f32.unop
define_instruction!(f32_abs, opcode::F32_ABS, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
        .try_into()
...
define_instruction!(f32_neg, opcode::F32_NEG, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: value::F32 = wasm
        .resumable
...
// f64.unop
define_instruction!(f64_abs, opcode::F64_ABS, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
        .try_into()
...
define_instruction!(f64_neg, opcode::F64_NEG, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: value::F64 = wasm
        .resumable
...
// i32.binop
define_instruction!(i32_add, opcode::I32_ADD, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
define_instruction!(i32_sub, opcode::I32_SUB, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: i32 = wasm
        .resumable
...
define_instruction!(i32_mul, opcode::I32_MUL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
define_instruction!(
    ...
    opcode::I32_DIV_S,
    |Args { wasm, .. }: &mut Args<T>| {
        let dividend: i32 = wasm
            .resumable
            .stack
            .pop_value()
            .try_into()
            .unwrap_validated();
        let divisor: i32 = wasm
            .resumable
            .stack
            .pop_value()
            .try_into()
...
define_instruction!(i32_and, opcode::I32_AND, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
define_instruction!(i32_or, opcode::I32_OR, |Args { wasm, .. }: &mut Args<T>| {
    let v1: i32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(i32_xor, opcode::I32_XOR, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
define_instruction!(i32_shl, opcode::I32_SHL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
// i64.binop
define_instruction!(i64_add, opcode::I64_ADD, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i64 = wasm
        .resumable
...
define_instruction!(i64_sub, opcode::I64_SUB, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: i64 = wasm
        .resumable
...
define_instruction!(i64_mul, opcode::I64_MUL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i64 = wasm
        .resumable
...
define_instruction!(
    ...
    opcode::I64_DIV_S,
    |Args { wasm, .. }: &mut Args<T>| {
        let dividend: i64 = wasm
            .resumable
            .stack
            .pop_value()
            .try_into()
            .unwrap_validated();
        let divisor: i64 = wasm
            .resumable
            .stack
            .pop_value()
            .try_into()
...
define_instruction!(i64_and, opcode::I64_AND, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: i64 = wasm
        .resumable
...
define_instruction!(i64_or, opcode::I64_OR, |Args { wasm, .. }: &mut Args<T>| {
    let v2: i64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(i64_xor, opcode::I64_XOR, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: i64 = wasm
        .resumable
...
define_instruction!(i64_shl, opcode::I64_SHL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: i64 = wasm
        .resumable
...
// f32.binop
define_instruction!(f32_add, opcode::F32_ADD, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
        .try_into()
...
define_instruction!(f32_sub, opcode::F32_SUB, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
...
define_instruction!(f32_mul, opcode::F32_MUL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
...
define_instruction!(f32_div, opcode::F32_DIV, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
...
define_instruction!(f32_min, opcode::F32_MIN, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
...
define_instruction!(f32_max, opcode::F32_MAX, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F32 = wasm
        .resumable
...
// f64.binop
define_instruction!(f64_add, opcode::F64_ADD, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
        .try_into()
...
define_instruction!(f64_sub, opcode::F64_SUB, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
...
define_instruction!(f64_mul, opcode::F64_MUL, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
...
define_instruction!(f64_div, opcode::F64_DIV, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
...
define_instruction!(f64_min, opcode::F64_MIN, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
...
define_instruction!(f64_max, opcode::F64_MAX, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v2: value::F64 = wasm
        .resumable
...
// i32.testop
define_instruction!(i32_eqz, opcode::I32_EQZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i32 = wasm
        .resumable
...
// i64.testop
define_instruction!(i64_eqz, opcode::I64_EQZ, |Args { wasm, .. }: &mut Args<
    T,
>| {
    let v1: i64 = wasm
        .resumable
...
// i32.relop
define_instruction!(i32_eq, opcode::I32_EQ, |Args { wasm, .. }: &mut Args<T>| {
    let v2: i32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(i32_ne, opcode::I32_NE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: i32 = wasm
        .resumable
        .stack
        .pop_value()
...
// i64.relop
define_instruction!(i64_eq, opcode::I64_EQ, |Args { wasm, .. }: &mut Args<T>| {
    let v2: i64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(i64_ne, opcode::I64_NE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: i64 = wasm
        .resumable
        .stack
        .pop_value()
...
// f32.relop
define_instruction!(f32_eq, opcode::F32_EQ, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f32_ne, opcode::F32_NE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f32_lt, opcode::F32_LT, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f32_gt, opcode::F32_GT, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f32_le, opcode::F32_LE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f32_ge, opcode::F32_GE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F32 = wasm
        .resumable
        .stack
        .pop_value()
...
// f64.relop
define_instruction!(f64_eq, opcode::F64_EQ, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f64_ne, opcode::F64_NE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f64_lt, opcode::F64_LT, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f64_gt, opcode::F64_GT, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f64_le, opcode::F64_LE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
...
define_instruction!(f64_ge, opcode::F64_GE, |Args { wasm, .. }: &mut Args<T>| {
    let v2: value::F64 = wasm
        .resumable
        .stack
        .pop_value()
