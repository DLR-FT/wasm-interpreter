...
impl<'wasm> WasmReader<'wasm> {
    ...
    pub fn read_var_u32(&mut self) -> Result<u32, ValidationError> {
        ...
        if has_next_byte {
            return Err(ValidationError::VariableLengthIntegerRepresentationTooLong);
        }

        let padding_bits_are_not_zero = byte & PADDING_IN_LAST_BYTE_BIT_MASK > 0;
        if padding_bits_are_not_zero {
            return Err(ValidationError::VariableLengthIntegerOverflowed);
        }
