...
pub enum ValidationError {
    ...
    ExpectedReferenceTypeOnValidationStack(ValType),
...
impl Display for ValidationError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            ValidationError::ExpectedReferenceTypeOnValidationStack(found_valtype) => write!(f, "Expected a reference type but instead found a `{found_valtype:?}` on the validation stack"),
