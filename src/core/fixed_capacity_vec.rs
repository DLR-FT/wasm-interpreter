...
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct FullContainerError;

impl From<FullContainerError> for crate::RuntimeError {
...
impl<T> FixedCapacityVec<T> {
    ...
    pub(crate) fn push(&mut self, value: T) -> Result<(), FullContainerError> {
        ...
        *self.elements.get_mut(self.len).ok_or(FullContainerError)? = MaybeUninit::new(value);
