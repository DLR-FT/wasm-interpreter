use criterion::{black_box, criterion_group, criterion_main, Criterion};

use wasm::{
    hooks::{EmptyHookSet, HookSet},
    validate, RuntimeInstance,
};

fn criterion_benchmark(c: &mut Criterion) {
    let wat = r#"
    (module
        (memory 1)
        (func $add_one (param $x i32) (result i32) (local $ununsed_local i32)
            local.get $x
            i32.const 1
            i32.add)

        (func $add (param $x i32) (param $y i32) (result i32)
            local.get $y
            local.get $x
            i32.add)

        (func (export "store_num") (param $x i32)
            i32.const 0
            local.get $x
            i32.store)
        (func (export "load_num") (result i32)
            i32.const 0
            i32.load)

        (export "add_one" (func $add_one))
        (export "add" (func $add))
    )
    "#;

    let wasm_bytes = wat::parse_str(wat).unwrap();

    let validation_info = validate(&wasm_bytes).unwrap();

    //
    // Set up an interpreter with the empty hook-set
    //
    let mut instance_empty_hookset: RuntimeInstance<'_, EmptyHookSet> =
        RuntimeInstance::new(&validation_info).unwrap();

    //
    // Set up an interpreter with an non-empty hook-set
    //
    #[derive(Default)]
    struct MyCustomHookSet;
    impl HookSet for MyCustomHookSet {
        fn instruction_hook(interpreter_state: &mut RuntimeInstance<Self>) {
            if black_box(interpreter_state.wasm_bytecode[0]) == 0x20 {
                eprintln!("got a local.get");
            }
        }
    }

    let mut instance_non_empty_hookset: RuntimeInstance<'_, MyCustomHookSet> =
        RuntimeInstance::new(&validation_info).unwrap();

    c.bench_function("invoke_func EmptyHookSet", |b| {
        b.iter(|| instance_empty_hookset.invoke_func::<_, ()>(black_box(2), black_box(42_i32)))
    });

    c.bench_function("invoke_func MyCustomHookSet", |b| {
        b.iter(|| instance_non_empty_hookset.invoke_func::<_, ()>(black_box(2), black_box(42_i32)))
    });
}

criterion_group!(benches, criterion_benchmark);
criterion_main!(benches);
